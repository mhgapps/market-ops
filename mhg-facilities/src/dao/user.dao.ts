...
export class UserDAO extends BaseDAO<'users'> {
   ...
   */
  async findByAuthUserId(authUserId: string, tenantId: string): Promise<User | null> {
  ...
  async findByLocation(locationId: string): Promise<User[]> {
    ...
    const { data, error } = (await supabase
      ...
      .is('deleted_at', null)
      .order('full_name', { ascending: true })) as unknown as QueryResult<User[]>
   ...
   */
  async createWithTenant(tenantId: string, userData: Omit<UserInsert, 'tenant_id'>): Promise<User> {
    const supabase = await getPooledSupabaseClient()
