...
type VendorRatingInsert =
  Database["public"]["Tables"]["vendor_ratings"]["Insert"];
...
export class VendorRatingDAO {
  ...
  private async getClient() {
    const supabase =
      (await getPooledSupabaseClient()) as SupabaseClient<Database>;
  ...
  async findByTicket(
    ticketId: string,
  ): Promise<VendorRatingWithRelations | null> {
    const { supabase } = await this.getClient();
  ...
  async getVendorStats(vendorId: string): Promise<VendorRatingStats | null> {
    ...
    const sumRating = ratings.reduce(
      (sum: number, r: VendorRating) => sum + (r.rating ?? 0),
      0,
    );
    ...
    const sumQuality = ratings.reduce(
      (sum: number, r: VendorRating) => sum + (r.quality_rating ?? 0),
      0,
    );
    const sumCost = ratings.reduce(
      (sum: number, r: VendorRating) => sum + (r.cost_rating ?? 0),
      0,
    );
  ...
  async getTopRatedVendors(limit = 10): Promise<VendorRatingStats[]> {
    ...
    const vendorIds = [
      ...new Set(
        data
          .map((r: { vendor_id: string | null }) => r.vendor_id)
          .filter(Boolean),
      ),
    ] as string[];
