...
export class TicketCommentDAO extends BaseDAO<'ticket_comments'> {
   ...
   */
  async findByTicketId(ticketId: string, includeInternal = false): Promise<TicketCommentWithUser[]> {
   ...
   */
  async countByTicketId(ticketId: string, includeInternal = false): Promise<number> {
    const { supabase } = await this.getClient()
   ...
   */
  async createComment(data: Omit<TicketCommentInsert, 'created_at' | 'deleted_at'>): Promise<TicketComment> {
    const { supabase } = await this.getClient()
