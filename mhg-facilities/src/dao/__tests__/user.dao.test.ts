...
describe('UserDAO', () => {
  ...
  describe('findByEmail', () => {
    ...
    it('should throw error for database errors', async () => {
      ...
      await expect(dao.findByEmail('test@example.com')).rejects.toThrow('Database error');
    });
  });
  ...
  describe('findByAuthUserId', () => {
    it('should find user by auth user ID and tenant', async () => {
      ...
      const result = await dao.findByAuthUserId('auth-user-id', 'test-tenant-id');

      expect(mockSupabase.auth.admin.getUserById).toHaveBeenCalledWith('auth-user-id');
      expect(mockQuery.eq).toHaveBeenCalledWith('tenant_id', 'test-tenant-id');
      expect(mockQuery.eq).toHaveBeenCalledWith('email', 'test@example.com');
      expect(result).toEqual(mockUser);
    });
  ...
  describe('findAdmins', () => {
    it('should return admin and super_admin users', async () => {
      ...
      const superAdminUser = { ...mockUser, id: 'user-2', role: 'super_admin' as UserRole };
      ...
      expect(mockQuery.in).toHaveBeenCalledWith('role', ['admin', 'super_admin']);
  ...
  describe('findByRole', () => {
    it('should return users with specific role', async () => {
      ...
      mockQuery.order.mockResolvedValueOnce({ data: [managerUser], error: null });
  ...
  describe('deactivate', () => {
    it('should deactivate user', async () => {
      ...
      mockQuery.single.mockResolvedValueOnce({ data: deactivatedUser, error: null });
    ...
    it('should throw error if user not found', async () => {
      ...
      await expect(dao.deactivate('non-existent')).rejects.toThrow('User not found');
    });
  });
  ...
  describe('reactivate', () => {
    it('should reactivate user', async () => {
      ...
      mockQuery.single.mockResolvedValueOnce({ data: reactivatedUser, error: null });
  ...
  describe('updateRole', () => {
    it('should update user role', async () => {
      ...
      mockQuery.single.mockResolvedValueOnce({ data: updatedUser, error: null });
