...
export class VendorDAO extends BaseDAO<"vendors"> {
  ...
  async findWithFilters(filters?: VendorFilters): Promise<Vendor[]> {
    ...
    if (filters) {
      ...
      if (filters.insurance_expiring_days !== undefined) {
        ...
        futureDate.setDate(
          futureDate.getDate() + filters.insurance_expiring_days,
        );
        query = query
      ...
      if (filters.contract_expiring_days !== undefined) {
        const futureDate = new Date();
        futureDate.setDate(
          futureDate.getDate() + filters.contract_expiring_days,
        );
        query = query
  ...
  async findWithFiltersPaginated(
    ...
    if (filters?.service_category) {
      countQuery = countQuery.contains("service_categories", [
        filters.service_category,
      ]);
      dataQuery = dataQuery.contains("service_categories", [
        filters.service_category,
      ]);
    }
    if (filters?.insurance_expiring_days !== undefined) {
