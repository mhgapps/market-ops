...
export type AttachmentType = 'initial' | 'progress' | 'completion' | 'invoice' | 'quote'
...
export class TicketAttachmentDAO extends BaseDAO<'ticket_attachments'> {
   ...
   */
  async findByTicketIdAndType(ticketId: string, attachmentType: AttachmentType): Promise<TicketAttachment[]> {
    const { supabase } = await this.getClient()
  ...
  async getTotalSizeByTicketId(ticketId: string): Promise<number> {
    ...
    return attachments?.reduce((total, att) => total + (att.file_size_bytes || 0), 0) ?? 0
  }

  /**
   * Create attachment with explicit types
   */
  async createAttachment(data: Omit<TicketAttachmentInsert, 'created_at' | 'deleted_at'>): Promise<TicketAttachment> {
    const { supabase } = await this.getClient()
