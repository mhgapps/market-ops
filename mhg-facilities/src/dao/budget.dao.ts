...
export class BudgetDAO extends BaseDAO<"budgets"> {
  ...
  async findByFiscalYear(fiscalYear: number): Promise<Budget[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to find budgets by fiscal year: ${error.message}`,
      );
    }
  ...
  async getBudgetSummary(fiscalYear: number): Promise<{
    ...
    const total_budget = budgets.reduce(
      (sum, b) => sum + (Number(b.annual_budget) || 0),
      0,
    );
    const total_spent = budgets.reduce(
   ...
   */
  async findByFiscalYearWithLocation(
    fiscalYear: number,
  ): Promise<BudgetWithLocation[]> {
    const { supabase, tenantId } = await this.getClient();
  ...
  async calculateSpendForBudget(
    ...
    if (category && category.toLowerCase() !== "total") {
      tickets = tickets.filter(
        (t) => t.category?.name?.toLowerCase() === category.toLowerCase(),
      );
    }
  ...
  async getSpendByLocation(fiscalYear: number): Promise<SpendByLocation[]> {
    ...
    tickets.forEach((t) => {
      if (t.location) {
        const current = locationMap.get(t.location.id) || {
