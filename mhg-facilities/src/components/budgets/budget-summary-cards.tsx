...
import type { BudgetSummaryEnriched, AlertLevel } from '@/services/budget.service'
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex flex-wrap md:flex-nowrap divide-y md:divide-y-0 md:divide-x divide-border">
          ...
          <div className="flex flex-col gap-1 px-4 py-3 flex-1 min-w-[50%] md:min-w-0 hover:bg-accent transition-colors">
            ...
            <span className="text-xl font-bold">{formatCurrency(summary.total_budget)}</span>
            <span className="text-xs text-muted-foreground">
              {summary.budget_count} budget{summary.budget_count !== 1 ? 's' : ''} active
            </span>
          </div>

          {/* Total Spent */}
          <div className={`flex flex-col gap-1 px-4 py-3 flex-1 min-w-[50%] md:min-w-0 hover:bg-accent transition-colors ${isOverBudget ? 'bg-red-50' : ''}`}>
          ...
          {/* Remaining */}
          <div className={`flex flex-col gap-1 px-4 py-3 flex-1 min-w-[50%] md:min-w-0 hover:bg-accent transition-colors ${remainingIsNegative ? 'bg-red-50' : ''}`}>
            <span className="text-sm text-muted-foreground">Remaining</span>
            <span
              className="text-xl font-bold"
              style={{ color: remainingIsNegative ? budgetUtilizationColors.over.text : undefined }}
            >
              {remainingIsNegative ? '-' : ''}{formatCurrency(Math.abs(summary.total_remaining))}
            </span>
            {remainingIsNegative && (
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="flex justify-between mt-2 text-xs text-muted-foreground">
            <span>
              {summary.warning_count + summary.danger_count + summary.over_budget_count} budget{summary.warning_count + summary.danger_count + summary.over_budget_count !== 1 ? 's' : ''} need attention
            </span>
