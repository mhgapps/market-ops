...
import type { CreateAssetCategoryInput, UpdateAssetCategoryInput } from '@/lib/validations/assets-vendors'
...
export function useAssetCategories() {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await api.get<{ categories: AssetCategoryWithParent[]; total: number }>(
...
export function useAssetCategoryTree() {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await api.get<{ tree: CategoryTreeNode[] }>('/api/asset-categories/tree')
      return data.tree
    },
  })
}
...
export function useCreateAssetCategory() {
  ...
  return useMutation({
    mutationFn: async (input: CreateAssetCategoryInput) => {
      const data = await api.post<{ category: AssetCategory }>('/api/asset-categories', input)
      return data.category
    },
    onSuccess: () => {
...
export function useUpdateAssetCategory() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateAssetCategoryInput }) => {
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['asset-categories'] })
      queryClient.invalidateQueries({ queryKey: ['asset-categories', variables.id] })
    },
  })
}
