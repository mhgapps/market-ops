...
export const budgetKeys = {
  ...
  summary: (fiscalYear?: number) => [...budgetKeys.all, 'summary', fiscalYear] as const,
  charts: () => [...budgetKeys.all, 'charts'] as const,
  categorySpend: (filters?: BudgetChartFilters) => [...budgetKeys.charts(), 'category', filters] as const,
  locationSpend: (fiscalYear?: number) => [...budgetKeys.charts(), 'location', fiscalYear] as const,
  monthlyTrend: (filters?: BudgetChartFilters) => [...budgetKeys.charts(), 'trend', filters] as const,
  yoyComparison: (filters?: BudgetChartFilters) => [...budgetKeys.charts(), 'yoy', filters] as const,
  forecasts: (fiscalYear?: number) => [...budgetKeys.all, 'forecasts', fiscalYear] as const,
  alerts: (fiscalYear?: number) => [...budgetKeys.all, 'alerts', fiscalYear] as const,
}
...
export function useUpdateBudget() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateBudgetData }) => {
      const response = await api.patch<{ budget: Budget }>(`/api/budgets/${id}`, data)
      return response.budget
    },
