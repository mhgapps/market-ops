  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.asset_type_id)
        params.set("asset_type_id", filters.asset_type_id);
      ...
      if (filters?.warranty_expiring_days)
        params.set(
          ...
          filters.warranty_expiring_days.toString(),
        );
...
export function useAsset(id: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const data = await api.get<{ asset: AssetWithRelations }>(
        `/api/assets/${id}`,
      );
      return data.asset;
    },
...
export function useUpdateAsset() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const result = await api.patch<{ asset: Asset }>(
        `/api/assets/${id}`,
        data,
      );
      return result.asset;
...
export function useTransferAsset() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const data = await api.post<{ transfer: unknown }>(
        `/api/assets/${assetId}/transfer`,
        {
          to_location_id,
          transferred_by,
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["assets", variables.assetId, "transfers"],
      });
    },
  });
}
