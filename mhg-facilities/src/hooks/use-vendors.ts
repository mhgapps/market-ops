  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.is_preferred !== undefined)
        params.set("is_preferred", filters.is_preferred.toString());
      ...
      if (filters?.insurance_expiring_days)
        params.set(
          ...
          filters.insurance_expiring_days.toString(),
        );
      if (filters?.contract_expiring_days)
        params.set(
          "contract_expiring_days",
          filters.contract_expiring_days.toString(),
        );
...
export function useUpdateVendor() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const result = await api.patch<{ vendor: Vendor }>(
        `/api/vendors/${id}`,
        data,
      );
...
export function useCreateVendorRating() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const data = await api.post<{ rating: VendorRating }>(
        `/api/vendors/${vendorId}/ratings`,
        {
          ticket_id,
          rating,
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["vendors", variables.vendorId, "ratings"],
      });
    },
  });
}
