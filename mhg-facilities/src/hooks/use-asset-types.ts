...
export function useAssetTypes(categoryId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const data = await api.get<{ assetTypes: AssetTypeWithCategory[]; total: number }>(
...
export function useCreateAssetType() {
  ...
  return useMutation({
    mutationFn: async (input: CreateAssetTypeInput) => {
      const data = await api.post<{ assetType: AssetType }>('/api/asset-types', input)
      return data.assetType
    },
    onSuccess: () => {
...
export function useUpdateAssetType() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['asset-types'] })
      queryClient.invalidateQueries({ queryKey: ['asset-types', variables.id] })
    },
  })
}
