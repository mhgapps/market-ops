...
export class BudgetService {
  ...
  async getBudgetWithSpend(id: string): Promise<BudgetWithSpend | null> {
    ...
    const utilization_percentage =
      annual > 0 ? Math.round((calculated_spent / annual) * 100) : 0;
   ...
   */
  async getBudgetsByFiscalYearWithSpend(
    fiscalYear: number,
  ): Promise<BudgetWithSpend[]> {
    const budgets =
      await this.budgetDAO.findByFiscalYearWithLocation(fiscalYear);
  ...
  async getSpendByCategory(
    ...
    const spendData = await this.budgetDAO.getSpendByCategory(
      fiscalYear,
      locationId,
    );
  ...
  async getUtilizationByLocation(fiscalYear: number): Promise<LocationSpend[]> {
    ...
    return spendData.map(({ location_id, location_name, spent }) => {
      ...
      const locationBudgets = budgets.filter(
        (b) => b.location_id === location_id,
      );
      const budget = locationBudgets.reduce(
        (sum, b) => sum + Number(b.annual_budget),
        0,
      );
  ...
  async getMonthlySpendTrend(
    ...
    const monthlyData = await this.budgetDAO.getMonthlySpend(
      fiscalYear,
      locationId,
    );
  ...
  async getYoYComparison(currentYear: number): Promise<{
    ...
    const currentMap = new Map(
      currentSpend.map((c) => [c.category_name, c.spent]),
    );
    const previousMap = new Map(
      previousSpend.map((c) => [c.category_name, c.spent]),
    );
    ...
    // Calculate totals
    const total_current = categories.reduce(
      (sum, c) => sum + c.current_year_spent,
      0,
    );
    const total_previous = categories.reduce(
      (sum, c) => sum + c.previous_year_spent,
      0,
    );
    const total_change_percentage =
      total_previous > 0
  ...
  async getForecastsForYear(fiscalYear: number): Promise<BudgetForecast[]> {
    ...
    return budgets.map((budget) => {
      ...
      const monthly_average =
        monthsElapsed > 0 ? budget.calculated_spent / monthsElapsed : 0;
      const projected_year_end = isCurrentYear
        ? monthly_average * 12
        : budget.calculated_spent;
      const will_exceed = projected_year_end > Number(budget.annual_budget);
      const projected_excess = will_exceed
        ? projected_year_end - Number(budget.annual_budget)
        : 0;
