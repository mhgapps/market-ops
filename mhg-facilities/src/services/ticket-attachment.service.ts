import {
  TicketAttachmentDAO,
  type AttachmentType,
} from "@/dao/ticket-attachment.dao";
...
export class TicketAttachmentService {
  ...
  async uploadAttachment(
    ...
    const totalSize =
      await this.attachmentDAO.getTotalSizeByTicketId(ticket_id);
    if (totalSize + file.size > MAX_TOTAL_SIZE_PER_TICKET) {
      throw new Error(
        `Total attachments size for this ticket would exceed ${MAX_TOTAL_SIZE_PER_TICKET / 1024 / 1024}MB`,
      );
    }
  ...
  async deleteAttachment(
    ...
    const attachment = (await this.attachmentDAO.findById(
      attachmentId,
    )) as TicketAttachment | null;
    if (!attachment) {
