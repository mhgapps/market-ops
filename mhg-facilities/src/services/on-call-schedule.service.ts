...
export class OnCallScheduleService {
   ...
   */
  async getSchedulesByDateRange(startDate: string, endDate: string): Promise<OnCallSchedule[]> {
  ...
  async createSchedule(data: CreateScheduleInput): Promise<OnCallSchedule> {
    ...
    if (!eligibleRoles.includes(user.role)) {
      throw new Error('Only staff, managers, and admins can be assigned on-call duties')
    }
    ...
    if (hasOverlap) {
      throw new Error('User already has an on-call schedule during this time period')
    }
   ...
   */
  async updateSchedule(id: string, data: UpdateScheduleInput): Promise<OnCallSchedule> {
    ...
    if (data.location_id !== undefined && data.location_id !== existingSchedule.location_id) {
      if (data.location_id) {
        const location = await this.locationDAO.findById(data.location_id)
        if (!location) {
          throw new Error('Location not found')
    ...
    if (data.user_id || data.start_date || data.end_date) {
      ...
      const hasOverlap = await this.scheduleDAO.hasOverlap(userId, newStartDate, newEndDate, id)
