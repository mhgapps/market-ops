...
type ComplianceDocumentType = Database['public']['Tables']['compliance_document_types']['Row'];
...
export class ComplianceDocumentTypeService {
  constructor(
    private docTypeDAO = new ComplianceDocumentTypeDAO()
  ) {}
  ...
  async createType(data: CreateDocTypeInput): Promise<ComplianceDocumentType> {
    ...
    return await this.docTypeDAO.create({
      ...
      is_location_specific: data.is_location_specific !== undefined ? data.is_location_specific : true,
    });
  }

  async updateType(id: string, data: UpdateDocTypeInput): Promise<ComplianceDocumentType> {
    ...
    if (data.name_es !== undefined) updateData.name_es = data.name_es?.trim() || null;
    if (data.description !== undefined) updateData.description = data.description?.trim() || null;
    if (data.default_alert_days !== undefined) updateData.default_alert_days = data.default_alert_days;
    if (data.renewal_checklist !== undefined) updateData.renewal_checklist = data.renewal_checklist;
    if (data.is_location_specific !== undefined) updateData.is_location_specific = data.is_location_specific;
