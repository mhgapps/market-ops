...
describe("LocationService", () => {
  ...
  describe("createLocation", () => {
    it("should create location successfully", async () => {
      ...
      when(mockLocationDAO.createLocation(anything())).thenResolve(
        mockLocation,
      );
    ...
    it("should throw error if manager not found", async () => {
      ...
      await expect(service.createLocation(input)).rejects.toThrow(
        "Manager not found",
      );
    });
  });
  ...
  describe("updateLocation", () => {
    it("should update location successfully", async () => {
      ...
      when(mockLocationDAO.findById("location-1")).thenResolve(mockLocation);
      when(
        mockLocationDAO.updateLocation("location-1", anything()),
      ).thenResolve({
        ...mockLocation,
        ...updates,
      });
  ...
  describe("assignManager", () => {
    it("should assign manager to location", async () => {
      ...
      when(mockUserDAO.findById("manager-1")).thenResolve(mockUser);
      when(
        mockLocationDAO.assignManager("location-1", "manager-1"),
      ).thenResolve({
        ...mockLocation,
        manager_id: "manager-1",
      });
