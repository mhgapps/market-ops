...
const attachmentTypes = ['initial', 'progress', 'completion', 'invoice', 'quote'] as const
...
export const holdTicketSchema = z.object({
  reason: z.string().min(10, 'Hold reason must be at least 10 characters').max(1000, 'Reason too long'),
})
...
export const createCommentSchema = z.object({
  comment: z.string().min(1, 'Comment cannot be empty').max(5000, 'Comment cannot exceed 5000 characters'),
