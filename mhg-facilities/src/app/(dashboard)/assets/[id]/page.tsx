    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 md:text-3xl">{asset.name}</h1>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <Badge className={getStatusColor(asset.status)}>
                {asset.status}
              </Badge>
              {asset.qr_code && (
                <code className="text-sm font-mono text-gray-600">{asset.qr_code}</code>
              )}
            </div>
          </div>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 text-yellow-800">
              ...
              <p className="font-medium">
                Warranty expires on {format(new Date(asset.warranty_expiration!), 'PPP')}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                {asset.warranty_expiration && (
                  <div>
                    <div className="text-sm text-gray-600">Warranty Expiration</div>
                    <div className="flex items-center gap-1">
                ...
                {asset.expected_lifespan_years && (
                  <div>
                    ...
                    <div className="font-medium">{asset.expected_lifespan_years} years</div>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          {transferData && transferData.transfers.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Transfer History ({transferData.transfer_count})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {((transferData.transfers ?? []) as Array<{
                    ...
                    <div key={transfer.id} className="flex items-start gap-3 border-l-2 border-gray-300 pl-4">
                      <ArrowRightLeft className="h-4 w-4 text-gray-500 mt-1" />
                      <div className="flex-1">
                        <div className="text-sm font-medium">
                          Transferred to {transfer.to_location?.name || 'Unknown'}
                        </div>
