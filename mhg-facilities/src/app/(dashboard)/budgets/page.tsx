...
const SpendByCategoryChart = dynamic(
  () =>
    import("@/components/budgets/charts/spend-by-category-chart").then(
      (mod) => ({ default: mod.SpendByCategoryChart }),
    ),
  {
    ssr: false,
...
const LocationUtilizationChart = dynamic(
  () =>
    import("@/components/budgets/charts/location-utilization-chart").then(
      (mod) => ({ default: mod.LocationUtilizationChart }),
    ),
  {
    ssr: false,
    loading: () => (
      <ChartSkeleton title="Utilization by Location" variant="horizontal-bar" />
    ),
  },
);
...
const MonthlyTrendChart = dynamic(
  () =>
    import("@/components/budgets/charts/monthly-trend-chart").then((mod) => ({
...
const YoYComparisonChart = dynamic(
  () =>
    import("@/components/budgets/charts/yoy-comparison-chart").then((mod) => ({
      default: mod.YoYComparisonChart,
    })),
  {
    ssr: false,
    loading: () => (
      <ChartSkeleton title="Year-over-Year Comparison" variant="bar" />
    ),
  },
);
import {
...
export default function BudgetsPage() {
  ...
  const [budgetToDelete, setBudgetToDelete] = useState<BudgetWithSpend | null>(
    null,
  );
  ...
  const { data: locationSpend, isLoading: locationLoading } = useLocationSpend(
    filters.fiscal_year,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <BudgetAlertsBanner
        ...
        onViewDetails={() =>
          setFilters((prev) => ({ ...prev, alert_level: "over" }))
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        />
        <YoYComparisonChart data={yoyData} isLoading={yoyLoading} />
      </div>
