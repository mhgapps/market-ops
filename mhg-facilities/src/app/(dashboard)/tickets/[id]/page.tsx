...
export default function TicketDetailPage({ params }: PageProps) {
  ...
  const { data: vendorsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{ data: Database['public']['Tables']['vendors']['Row'][], total: number }>('/api/vendors?is_active=true')
  ...
  const handleStatusAction = async (action: string, data?: { cost?: number; notes?: string; new_status?: string }) => {
    try {
          ...
          <div>
            ...
            {isEditingTitle ? (
              <div className="mt-2 flex items-center gap-2">
                ...
                />
                <Button size="sm" variant="ghost" onClick={saveTitle} disabled={updateTicket.isPending}>
              ...
              <CardContent className="space-y-4">
                ...
                {ticket.contained_at ? (
                  <div className="flex items-center gap-2 text-green-700">
                    ...
                    <span>Contained on {format(new Date(ticket.contained_at), 'MMM d, yyyy h:mm a')}</span>
                  </div>
                ) : (
                  <div className="flex items-center justify-between">
                    <span className="text-red-700">Emergency not yet contained</span>
                    {(canManage || isAssignedToCurrentUser) && ticket.status === 'submitted' && (
                      <Button
                        variant="outline"
                ...
                {ticket.status === 'closed' && ticket.resolution_notes ? (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-green-700">
                      ...
                      <span>Resolved on {ticket.closed_at ? format(new Date(ticket.closed_at), 'MMM d, yyyy h:mm a') : 'N/A'}</span>
                    </div>
                    <div className="rounded-md bg-white p-3">
                      <p className="text-sm font-medium text-gray-700">Resolution Notes:</p>
                      <p className="mt-1 text-sm text-gray-600">{ticket.resolution_notes}</p>
                    </div>
                  </div>
                ) : ticket.contained_at && (ticket.status === 'in_progress' || ticket.status === 'completed') ? (
                  showResolveForm ? (
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            if (resolutionNotes.trim()) {
                              await statusAction.mutateAsync({ action: 'resolve', notes: resolutionNotes })
                              setShowResolveForm(false)
                              setResolutionNotes('')
                            }
                          }}
                          disabled={statusAction.isPending || !resolutionNotes.trim()}
                        >
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-amber-700">Emergency contained, awaiting resolution</span>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {canManage && !isEditingDescription && (
                <Button size="sm" variant="ghost" onClick={startEditingDescription}>
                  <Pencil className="h-3.5 w-3.5" />
                </Button>
              )}
            </CardHeader>
              ...
              {isEditingDescription ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button size="sm" onClick={saveDescription} disabled={updateTicket.isPending}>
                      {updateTicket.isPending ? 'Saving...' : 'Save'}
                    </Button>
                  </div>
                </div>
              ) : (
                <p className="whitespace-pre-wrap text-gray-700">{ticket.description || 'No description provided.'}</p>
              )}
            </CardContent>
          </Card>
