...
import { Plus, Search, LayoutList, LayoutGrid, AlertTriangle, FileText } from 'lucide-react'
...
export function TicketsPageContent() {
  ...
  const filters = {
    ...
    ...(emergencyFilter !== 'all' && { is_emergency: emergencyFilter === 'emergency' }),
  ...
  const handleDragEnd = useCallback(async (result: { destination?: { droppableId: string } | null; draggableId: string }) => {
    ...
    const previousData = queryClient.getQueryData(ticketKeys.list(currentFilters))

    queryClient.setQueryData(ticketKeys.list(currentFilters), (old: typeof previousData) => {
      if (!old || !('data' in (old as object))) return old
      const oldData = old as { data: Ticket[]; total: number; page: number; pageSize: number; totalPages: number }
      return {
        ...oldData,
        ...
        <div className="flex flex-wrap md:flex-nowrap divide-y md:divide-y-0 md:divide-x divide-border">
          ...
          >
            <AlertTriangle className={`h-4 w-4 ${
              stats.emergencyCount > 0 ? 'text-red-500' : 'text-muted-foreground'
            }`} />
        ...
        <TableLoadingOverlay isLoading={isFetching}>
          <DragDropContext onDragEnd={handleDragEnd}>
            <div className="flex gap-4 overflow-x-auto pb-4">
              {(Object.entries(kanbanColumns) as [TicketStatus, Ticket[]][]).map(
