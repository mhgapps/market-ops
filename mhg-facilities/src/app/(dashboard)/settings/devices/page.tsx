...
function DeviceIcon({ deviceName, className }: { deviceName: string | null; className?: string }) {
  const name = (deviceName ?? '').toLowerCase()
  if (name.includes('iphone') || name.includes('ios') || name.includes('android')) {
      ...
      <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="rounded-lg bg-primary/10 p-2">
            <DeviceIcon deviceName={device.device_name} className="h-5 w-5 text-primary" />
          </div>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {device.is_current ? 'Sign out of this device?' : 'Revoke device trust?'}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {device.is_current
...
export default function TrustedDevicesPage() {
  ...
  const loadDevices = useCallback(async () => {
    try {
      const { devices: data } = await api.get<{ devices: DeviceInfo[] }>('/api/auth/devices')
      setDevices(data)
    } catch {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            Devices that can sign in without your password. Each device is trusted for 180 days.
          </p>
        </div>
