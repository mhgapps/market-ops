...
import {
  markAsRenewedSchema,
  markAsConditionalSchema,
  markAsFailedInspectionSchema,
  ...
  try {
    ...
    switch (action) {
      case "mark_renewed": {
        ...
        document = await service.markAsRenewed(
          id,
      ...
      case "mark_conditional": {
        ...
        document = await service.markAsConditional(
          id,
      ...
      case "mark_failed_inspection": {
        ...
        document = await service.markAsFailedInspection(
          id,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : String(error) || "Failed to update compliance status",
      },
      { status: 500 },
    );
  }
